...
答案：原`exercise1.10`已经考虑了大小顺序问题。

## 练习1.20

> 在网站http://www.informit.com/title/032174113 上，第1章的代码目录包含了头文件 Sales_item.h。将它拷贝到你自己的工作目录中。用它编写一个程序，读取一组书籍销售记录，将每条记录打印到标准输出上。

详见`exercise1_20.cpp`，可使用输入文件重定向方便输入`./exercise1_20 <book.txt`，其中`book.txt`是实现写好的输入，位于可执行文件的统一目录下。
...
详见`exercise1_21.cpp`，输入可以复制`book.txt`的前两行。
...
题目意思是输入类似“两个相同的或者三个相同的、五个相同的”这种，不确定输入的个数。如果有不同ISBN的，则退出程序。
...
使用输入重定向功能`./exercise1_23 <book.txt`。
